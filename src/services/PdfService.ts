...
export class PdfService{
  public async exportPdf (id: number, firstName:string, lastName:string, locale: string){
    const response = await pdfshift(process.env.NEXT_PUBLIC_PDF_SERVICE_KEY as string, {source:'https://meoagent.com/'+ locale +'/profile/realtor/' + id + '?pdf=1', delay: (5 * 1000), pages:'1', format:'350mmx1094mm'}) as  { data: any }
